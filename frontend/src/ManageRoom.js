...
function ManageRoom() {
  ...
  const handleReport = async (appliance, status) => {
    try {
      ...
      const data = await response.json();
      alert(data.success ? 'Report successfully submitted' : 'Failed to submit report: ' + data.message);
    } catch (error) {
      console.error('Error submitting report:', error);
      alert('An error occurred while submitting the report.');
    }
    ...
    <div>
        ...
        <div className="ac-status">
          ...
          <div className="toggle-container">
            <button 
              ...
              disabled={!isOn}
              style={{ backgroundColor: isOn ? '' : 'gray', color: isOn ? '' : 'white' }}
            >
              Manual
            </button>
            <button 
      ...
      <div className="buttons-container">
        {['All', 'Appliances', 'Sensors'].map(category => (
          <button
            key={category}
            className={`buttons-style ${activeCategory === category ? 'active' : ''}`}
